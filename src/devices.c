...
static inline size_t get_hierarchy_sizeof_type(uint16_t type) {
    switch(type) {
        default:
        case XCB_INPUT_HIERARCHY_CHANGE_TYPE_ADD_MASTER:
            return sizeof(((xcb_input_hierarchy_change_data_t*)0)->add_master) - sizeof(((xcb_input_hierarchy_change_data_t*)0)->add_master.name);
        case XCB_INPUT_HIERARCHY_CHANGE_TYPE_REMOVE_MASTER:
            return sizeof(((xcb_input_hierarchy_change_data_t*)0)->remove_master);
        case XCB_INPUT_HIERARCHY_CHANGE_TYPE_ATTACH_SLAVE:
            return sizeof(((xcb_input_hierarchy_change_data_t*)0)->attach_slave);
        case XCB_INPUT_HIERARCHY_CHANGE_TYPE_DETACH_SLAVE:
            return sizeof(((xcb_input_hierarchy_change_data_t*)0)->detach_slave);
    }
}
static inline xcb_input_hierarchy_change_t* create_hierarchy_change_struct(uint16_t type, xcb_input_hierarchy_change_data_t** data_ptr) {
    ...
    change->type = type;
    change->len = (sizeof(xcb_input_hierarchy_change_t) + get_hierarchy_sizeof_type(type)) /sizeof(uint32_t);
    *data_ptr = data;
    return change;
}
