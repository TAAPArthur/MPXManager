...
void sendDeviceAction(MasterID id, int detail, int type, WindowID window) {
    LOG(LOG_LEVEL_VERBOSE, "Sending action: id %d detail %d type %d to win %d\n", id, detail, type, window);
    assert(detail);
    assert(id < 100);
...
void sendButtonPress(int button, MasterID id, WindowID win) {
    sendDeviceAction(id ? id : getActiveMasterPointerID(), button, XCB_INPUT_BUTTON_PRESS, win);
}
void sendButtonRelease(int button, MasterID id, WindowID win) {
    sendDeviceAction(id ? id : getActiveMasterPointerID(), button, XCB_INPUT_BUTTON_RELEASE, win);
}
...
void sendKeyPress(int keycode, MasterID id, WindowID win) {
    sendDeviceAction(id ? id : getActiveMasterKeyboardID(), keycode, XCB_INPUT_KEY_PRESS, win);
}
void sendKeyRelease(int keycode, MasterID id, WindowID win) {
    sendDeviceAction(id ? id : getActiveMasterKeyboardID(), keycode, XCB_INPUT_KEY_RELEASE, win);
}
