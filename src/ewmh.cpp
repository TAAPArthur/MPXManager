...
void addEWMHRules(AddFlag flag) {
    ...
    getEventRules(DEVICE_EVENT).add(DEFAULT_EVENT(detectWindowMoveResizeButtonRelease), flag);
...
bool detectWindowMoveResizeButtonRelease(Master* m) {
    auto ref = getRef(m);
    if(ref) {
        if(ref->btn && ref->btn == getLastUserEvent().detail &&
            getLastUserEvent().mask == XCB_INPUT_XI_EVENT_MASK_BUTTON_RELEASE) {
...
void updateWindowMoveResize(Master* m) {
    auto ref = getRef(m);
    if(ref) {
        DEBUG("Updating WM move/resize; Master: " << m->getID());
        if(!ref->shouldUpdate())
            return;
        short pos[2];
