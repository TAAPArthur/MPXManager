...
static ArrayList<Layout*> registeredLayouts = ArrayList<Layout*>(DEFAULT_LAYOUTS, LEN(DEFAULT_LAYOUTS));


Layout* findLayoutByName(std::string name) {
    for(Layout* layout : registeredLayouts)
...
static void applyMasksToConfig(const Monitor* m, int* values, WindowInfo* winInfo) {
    if(winInfo->hasMask(ROOT_FULLSCREEN_MASK)) {
        ...
        for(int i = 0; i < 4; i++)
            values[CONFIG_X + i] = (&m->getBase().x)[i];
    }
    else {
        if(winInfo->hasMask(X_MAXIMIZED_MASK))
            values[CONFIG_WIDTH] = m->getViewport().width;
        if(winInfo->hasMask(Y_MAXIMIZED_MASK))
            values[CONFIG_HEIGHT] = m->getViewport().height;
    }
}
...
static int adjustBorders(LayoutState* state, WindowInfo* winInfo, int config[CONFIG_LEN], int mask) {
    if(winInfo->hasMask(INPUT_ONLY_MASK)) {
        ...
        config[CONFIG_BORDER] = state->getArgs()->noBorder ? 0 : DEFAULT_BORDER_WIDTH;
...
void configureWindow(LayoutState* state, WindowInfo* winInfo, const short values[CONFIG_LEN]) {
    ...
    config[CONFIG_STACK] = state->getArgs() && state->getArgs()->lowerWindows ? XCB_STACK_MODE_BELOW : XCB_STACK_MODE_ABOVE;
    ...
    if(winInfo->isTilingOverrideEnabledAtIndex(CONFIG_BORDER))
        config[CONFIG_BORDER] = MAX(0, tilingOverride.border);
    ...
    for(int i = 0; i < list.size(); i++) {
        ...
        if(winInfo->isInteractable() && !winInfo->isTileable())
            if(winInfo->hasPartOfMask(MAXIMIZED_MASK | FULLSCREEN_MASK | ROOT_FULLSCREEN_MASK)) {
                ...
                int border = winInfo->isTilingOverrideEnabledAtIndex(CONFIG_BORDER) ? winInfo->getTilingOverride()[CONFIG_BORDER] :
